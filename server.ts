...
const PORT = 9080;
const ROOM_SECRETS_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
...
function randomSecret(attempts: number) {
    let out = '';
    for (let i = 0; i < 4 + attempts; i++) {
        out += ROOM_SECRETS_ALPHABET[randomInt(0, ROOM_SECRETS_ALPHABET.length - 1)];
    }
    return out;
}

function ProtoMessage(type: number, id: number, data: string | undefined) {
...
class Lobby {
    ...
    constructor(name: string, host: number, mesh: boolean) {
        this.name = name;
        this.host = host;
        this.mesh = mesh;
        this.peers = [];
    ...
    join(peer: Peer) {
        ...
        this.peers.forEach((p) => {
            p.ws.send(ProtoMessage(CMD.PEER_CONNECT, assigned, undefined));
            peer.ws.send(ProtoMessage(CMD.PEER_CONNECT, this.getPeerId(p), undefined));
        });
    ...
    leave(peer: Peer) {
        ...
        this.peers.forEach((p) => {
            if (close) { // Room host disconnected, must close.
                ...
                p.ws.send(ProtoMessage(CMD.PEER_DISCONNECT, assigned, undefined));
            }
        });
        this.peers.splice(idx, 1);
        if (close && this.closeTimer != null) {
    ...
    seal(peer: Peer) {
        ...
        this.sealed = true;
        this.peers.forEach((p) => {
            p.ws.send(ProtoMessage(CMD.SEAL, 0, undefined));
        });
...
function joinLobby(peer: Peer, pLobby: string, mesh: boolean) {
    ...
    if (lobbyName === '') {
        ...
        let attempts = 0;
        lobbyName = randomSecret(attempts);
